...
if (!file.exists(file_path)) {
  download.file("https://ftp.pride.ebi.ac.uk/pride/data/archive/2016/09/PXD002918/proteinGroups.txt", file_path)
}
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = timepoint, shape = condition)) +
  geom_point(size = 4) +
  theme_minimal() +
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = condition, shape = timepoint)) +
  geom_point(size = 4) +
...
pheatmap(cor_matrix,
         annotation_col = sample_metadata[, c("condition", "timepoint"), drop = FALSE],
         annotation_row = sample_metadata[, c("condition", "timepoint"), drop = FALSE],
